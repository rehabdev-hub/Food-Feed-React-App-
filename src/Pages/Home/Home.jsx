...
import {
  ...
  FaRegNewspaper,
  FaChartBar,
  FaShareAlt, // (kept import in case your CSS references it somewhere)
} from "react-icons/fa";
...
function Home() {
    ...
    async function loadSuggestions() {
      ...
      const filtered = (profs || [])
        ...
        .map((p) => ({
          ...
          avatar:
            bust(toPublicUrl(AVATAR_BUCKET, p.avatar_url)) || fallbackAvatar,
  ...
  const goToMyProfile = () => {
    // Self profile: simpler route (adjust if your app expects /profile?id=me)
    navigate("/profile");
  };

  const goToUserProfile = (id) => {
    // Other users: use the id query param (your Profile supports /profile?id=...)
    if (!id) return;
